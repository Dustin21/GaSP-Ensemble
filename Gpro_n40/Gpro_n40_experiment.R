...
#---------------- Data Inputs ------------------------
#-----------------------------------------------------
gpro <- read.csv("gpro.csv", header = TRUE)

# split data into list of data.frames
nVars <- ncol(gpro)
x.train <- split(gpro[,-nVars], (seq(nrow(gpro)) - 1) %/% 40)
y.train <- split(gpro[,nVars], (seq(nrow(gpro)) - 1) %/% 40)

# test set
gpro.test <- read.csv("gpro_test.csv", header = TRUE)
