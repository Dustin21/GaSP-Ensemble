...
#---------------- Data Inputs ------------------------
#-----------------------------------------------------
gpro <- read.csv("ptw.csv", header = TRUE)

# split data into list of data.frames
nVars <- ncol(gpro)
x.train <- split(gpro[,-nVars], (seq(nrow(gpro)) - 1) %/% 220)
y.train <- split(gpro[,nVars], (seq(nrow(gpro)) - 1) %/% 220)

# test set
gpro.test <- read.csv("ptw_test.csv", header = TRUE)
